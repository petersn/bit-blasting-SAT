...
class Instance(object):
	...
	def to_dimacs(self):
		lines = ["c Generated by solver.py\n"]
		all_variables = self.get_all_variables()
		lines.append("p cnf %i %i\n" % (len(all_variables), len(self.clauses)))
		for clause in self.clauses:
			clause_numbers = [i for i in clause.positive] + [-i for i in clause.negative] + [0]
			lines.append(" ".join(map(str, clause_numbers)) + "\n")
		return "".join(lines)
...
def random_instance(var_count, clause_count, valid_clause_lengths):
	...
	for _ in xrange(clause_count):
		...
		clause_length = random.choice(valid_clause_lengths)
		variables = random.sample(xrange(1, var_count + 1), clause_length)
		for v in variables:
			random.choice((positive, negative)).add(v)
